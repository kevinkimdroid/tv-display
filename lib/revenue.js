...
const { buildBudgetComparison, ytdMonthCount } = require('./budgets');
...
function buildDashboard(year, raw) {
  const amountByCode = Object.fromEntries(
    raw.accounts.map(([code, amount]) => [String(code), toPositive(amount)])
  );

  // Always expose every configured product, even if Oracle has no rows yet.
  const accounts = ACCOUNT_NUMBERS.map((code) => ({
    code,
    name: ACCOUNT_NAMES[code] || 'Unknown Account',
    amount: amountByCode[code] || 0
  }));

  const topAccounts = [...accounts].sort((a, b) => b.amount - a.amount);
  ...
  const budget = buildBudgetComparison(year, monthlyByAccount, monthlyMap, accounts);

  // Match portfolio "Actual (Posted)" — sum posted monthly totals, not per-account abs().
  // Summing abs() per GL account understates the true YTD when nets offset across accounts.
  const ytdTotal = budget?.summary?.actualYtd != null
    ? budget.summary.actualYtd
    : MONTH_ORDER.slice(0, ytdMonthCount(year, monthlyMap))
        .reduce((sum, p) => sum + (Number(monthlyMap[p]) || 0), 0);
