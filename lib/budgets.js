...
function ytdMonthCount(year, oracleMonthly = {}) {
  const now = new Date();
  const y = Number(year);
  // Past fiscal years are fully closed.
  if (Number.isFinite(y) && y < now.getFullYear()) return 12;

  // Prefer months that already have Oracle postings (true posted YTD).
  const posted = MONTH_ORDER.filter((p) => Number(oracleMonthly[p]) > 0).length;
  if (posted > 0) return posted;

  // Fallback: include current month (getMonth is 0-based).
  return Math.min(Math.max(now.getMonth() + 1, 1), 12);
}
...
function buildBudgetComparison(year, monthlyByAccount, oracleMonthly = {}, glAccounts = []) {
  ...
  const monthCount = ytdMonthCount(year, oracleMonthly);
  const periods = MONTH_ORDER.slice(0, monthCount);
  const throughLabel = !periods.length
    ? 'No posted months yet'
    : periods.length === 1
      ? periods[0]
  ...
  // Full Oracle posted YTD = sum of every month that has postings (true total premium).
  const oracleActualYtd = periods.reduce(
    (sum, p) => sum + (Number(oracleMonthly[p]) || 0),
    0
  );
  const overallBudgetYtd = file.overall
    ? sumMonths(file.overall.months, monthCount)
  ...
  const summary = {
    ...
    actualLabel: 'Oracle posted YTD total premium',
...
module.exports = { buildBudgetComparison, loadBudgetFile, ACCOUNT_META, ytdMonthCount };
